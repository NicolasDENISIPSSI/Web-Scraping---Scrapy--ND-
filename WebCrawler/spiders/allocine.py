...
class AllocineSpider(scrapy.Spider):
    ...
    start_urls = [
        f'https://www.allocine.fr/film/meilleurs/?page={n}' for n in range(1, 10)]
        ...
        for film in liste_film:
            ...
            try:
                item['author'] = film.css(
                    'a.blue-link::text')[0].extract().strip()
            except:
                item['author'] = 'None'
            ...
            try:
                item['time'] = film.css(
                    'div.meta-body-item.meta-body-info::text')[0].extract().strip('\n')
            except:
                item['time'] = 'None'
            ...
            try:
                item['genre'] = film.css(
                    '.meta-body-item.meta-body-info')[0].css('span::text')[1:4].extract()
            except:
                item['genre'] = 'None'
            ...
            try:
                item['score'] = film.css(
                    'span.stareval-note::text')[1].extract()
            except:
                item['score'] = 'None'
            ...
            try:
                item['desc'] = film.css(
                    'div.content-txt::text')[0].extract().strip()
            except:
                item['desc'] = 'None'
